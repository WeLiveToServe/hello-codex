...
def spinner(stop_event, recording_event, paused_event):
    ...
    while not stop_event.is_set():
        if recording_event.is_set():
            sys.stdout.write(f"\r{RED}🔴 RECORDING...{RESET} {symbols[i % len(symbols)]}  (release SPACE to pause)    ")
        elif paused_event.is_set():
            sys.stdout.write(f"\r{YELLOW}⏸️  PAUSED{RESET} (hold SPACE to continue, ESC to finish)    ")
        else:
...
def record_push_to_talk():
    ...
    print("\n" + "="*60)
    print("PUSH-TO-TALK RECORDER (pause/resume enabled)")
    print("="*60)
...
def main():
    ...
    if outpath:
        playback(outpath)
        print(f"Saved recording to {outpath}")
        return outpath
    return None
